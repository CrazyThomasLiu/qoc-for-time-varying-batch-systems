...
df_K_optimal = pd.read_csv('./Data/MIMO/optimal_control_law.csv', index_col=0)
K_optimal = df_K_optimal.to_numpy()

"2. load the initial control law K_initial from the csv"
df_K_initial = pd.read_csv('./Data/MIMO/initial_control_law.csv', index_col=0)
...
for ite in range(iteration_num):
    df_K_q_learning=pd.read_csv('./Data/MIMO/policy_iteration_25/q_learning_control_policy{}.csv'.format(ite+1), index_col=0)
...
if save_figure==True:
    #plt.savefig('Compare_pi_MIMO_final.pdf')
    plt.savefig('Compare_pi_MIMO_final.jpg',dpi=800)
plt.show()
