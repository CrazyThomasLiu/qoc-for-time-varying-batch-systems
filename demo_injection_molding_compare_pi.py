...
for ite in range(iteration_num):
    df_K_q_learning=pd.read_csv('./Data/Injection_Molding/policy_iteration_10/q_learning_control_policy{}'.format(ite+1), index_col=0)
...
if save_figure==True:
    #plt.savefig('Compare_pi_injection_molding.jpg', dpi=800)
    plt.savefig('Compare_pi_injection_molding.pdf')
    #plt.savefig('Compare_pil.pdf')
plt.show()
